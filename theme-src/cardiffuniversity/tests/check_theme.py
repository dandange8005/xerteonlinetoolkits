#!/usr/bin/env python3
"""Computed-style checks for the cardiffuniversity theme (headless Chrome, standard library only).
...
def header_markup(left, right):
    ...
    return (f'<header class="{cls}" id="overview"><div class="container">'
            + slot("L", "../../../themes/site/cardiffuniversity/logo_left.svg", left) + slot("R", PARTNER_LOGO, right)
...
CHECKS = [
    ...
    ("header", "no top rule (dropped 24 September)", "cs('#overview','borderTopWidth')", "0px"),
    ...
    # cardiffuniversity.js publishes --cu-sticky-nav (the sticky page menu's height). These assert
    ...
    # cardiffuniversity.js: the player makes #topnav sticky when the page menu sits above the header,
    ...
    ("stickynav", "theme script is loaded and exposes its hook", "String(typeof window.cardiffuniTheme === 'object')", "true"),
    ("stickynav", "no sticky page menu means no offset to clear",
     "(function(){window.cardiffuniTheme.apply();"
    ...
    ("stickynav", "a sticky bar above the header is measured",
     ...
     "window.cardiffuniTheme.apply();"
     "var v=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--cu-sticky-nav'));"
     "n.style.position='';n.style.top='';window.cardiffuniTheme.apply();return String(v>0)})()", "true"),
    ("stickynav", "a sticky #pageLinks wrapper below the header is measured too",
     ...
     "window.cardiffuniTheme.apply();"
     "var v=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--cu-sticky-nav'));"
     "w.style.position='';w.style.top='';window.cardiffuniTheme.apply();return String(v>0)})()", "true"),
    ("stickynav", "the section menu gets an inline top that beats the player's",
     ...
     "m.style.top='65px';w.style.position='sticky';w.style.top='0';window.cardiffuniTheme.apply();"
     "var ok=m.style.top!=='65px'&&parseFloat(m.style.top)>0;"
     "w.style.position='';w.style.top='';window.cardiffuniTheme.apply();m.style.top='';return String(ok)})()", "true"),
    ...
    ("stickynav", "the spy offset follows the sticky bar too",
     "(function(){var w=document.getElementById('pageLinks');w.style.position='sticky';w.style.top='0';"
     "window.cardiffuniTheme.apply();var withBar=window.__spy.options.offset;"
     "w.style.position='';w.style.top='';window.cardiffuniTheme.apply();"
    ...
    ("stickynav", "the script corrects a landing instantly, not with a second animation",
     ...
     "location.hash='#test-section';window.cardiffuniTheme.settleOnTarget();"
    ...
    ("stickynav", "the script follows the player's content lifecycle",
     ...
     "finally{n.style.position='';n.style.top='';window.cardiffuniTheme.apply();}})()", "true"),
    ...
    ("stickynav", "a bar that changes height is re-measured",
     ...
     "n.querySelector('.navbar-inner').appendChild(pad);window.cardiffuniTheme.apply();"
     ...
     "pad.remove();n.style.position='';n.style.top='';window.cardiffuniTheme.apply();"
    ...
    ("stickynav", "resize observer watches both navbar candidates and schedules measurement",
     ...
     "finally{pad.remove();n.style.position='';n.style.top='';window.cardiffuniTheme.apply();}})()", "true"),
    ...
    ("stickynav", "dragging the scrollbar stops the script re-landing the page",
     "(function(){location.hash='#test-section';window.cardiffuniTheme.startSettling();"
     "var before=window.cardiffuniTheme.state().settling;"
     "window.dispatchEvent(new MouseEvent('mousedown'));"
     "var after=window.cardiffuniTheme.state().settling;location.hash='';"
     "return String(before === true && after === false)})()", "true"),
    # Change the shared source and exercise old aliases and newer consumers together.
    ("theming", "brand role updates legacy utilities",
     "withRootTokens({'--color-brand-primary':'rgb(0, 80, 140)'},()=>classStyle('bg-brand','backgroundColor'))",
     "rgb(0, 80, 140)"),
    ("theming", "the brand primary recolours both navbar markers and the side-menu rule",
     "withBrand('rgb(0, 112, 60)','rgb(29, 43, 79)',()=>[cs('#pageLinks .navbar-inner','borderBottomColor'),cs('#nav li.activePage a','boxShadow').match(/rgb\\([^)]*\\)/)[0],cs('#toc > li.active > a','borderLeftColor')].join(' | '))",
     "rgb(0, 112, 60) | rgb(0, 112, 60) | rgb(0, 112, 60)"),
